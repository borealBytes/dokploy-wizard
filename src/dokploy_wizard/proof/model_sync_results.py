...
from dokploy_wizard.proof import REQUIRED_RESULT_KEYS
from dokploy_wizard.proof.model_sync_artifacts import (
    CaptureSchemaError,
    JsonValue,
    read_exact_regular_bytes,
    sha256_bytes,
...
if TYPE_CHECKING:
    from dokploy_wizard.proof.model_sync_host_a import BaselineArtifactInputs
...
def validate_attestation(attestation: proof.BaselineAttestation) -> None:
    try:
        build_result({**attestation.result_body, "abort_guard_sha256": "f" * 64})
        proof.validate_baseline_attestation_bindings(attestation)
    except ValueError as error:
        raise ValueError("abort guard attestation result body is invalid") from error


def derive_result_from_attestation(attestation: proof.BaselineAttestation) -> dict[str, JsonValue]:
    validate_attestation(attestation)
    return build_result({**attestation.result_body, "abort_guard_sha256": proof.abort_guard_sha256(attestation)})


def result_bytes_from_attestation(attestation: proof.BaselineAttestation) -> bytes:
    return proof.canonical_json_bytes(derive_result_from_attestation(attestation)) + b"\n"


def baseline_result_values(
    ...
    return {
        ...
        "external_backup_path": str(inputs.prepared.backup_path.resolve()),
        "abort_guard_path": str(inputs.guard_path.resolve()),
        "abort_guard_sha256": guard_hash,
        "host_a_preflight_sha256": sha256_bytes(payloads["host-a-preflight.json"]),
        "host_b_preflight_sha256": sha256_bytes(payloads["host-b-preflight.json"]),
        "host_identities_distinct": True,
        "host_architectures_equal": True,
        "baseline_sha256": sha256_bytes(payloads["baseline.json"]),
        "protected_artifacts_before_path": str(
            (inputs.artifact_dir / "protected-artifacts-before.txt").resolve()
        ),
        "protected_artifacts_before_sha256": sha256_bytes(manifest),
...
def require_generated_bounds(payloads: Mapping[str, bytes], result: bytes) -> None:
    if any(len(content) > _MAX_DATA_OUTPUT_BYTES for content in payloads.values()):
        raise proof.AbortGuardError("generated data output exceeds its bound")
    if len(result) > _MAX_RESULT_BYTES:
        raise proof.AbortGuardError("generated result output exceeds its bound")


def _read_proof_bytes(path: Path, max_bytes: int, mode: int) -> bytes:
    try:
        return proof.read_bounded_regular_bytes(path, max_bytes, mode)[0]
    except (OSError, ValueError) as error:
        raise proof.AbortGuardError("proof file is not a stable bounded regular file") from error
...
def protected_bytes(paths: ProofRecoveryPaths) -> bytes:
    ...
    try:
        manifest = proof.read_bounded_regular_bytes(
            manifest_path, _MAX_PROTECTED_MANIFEST_BYTES, 0o600
        )[0]
        entries = validate_protected_manifest_bytes(manifest)
        proof.verify_protected_artifacts(paths.repository_root, entries)
        receipt = proof.read_bounded_regular_bytes(
            receipt_path, _MAX_PROTECTED_RECEIPT_BYTES, 0o600
        )[0]
    except (CaptureSchemaError, OSError, ValueError) as error:
        raise proof.AbortGuardError("protected manifest contract is invalid") from error
    expected = f"{sha256_bytes(manifest)}  protected-artifacts-before.txt\n".encode()
    if receipt != expected:
        raise proof.AbortGuardError("pre-existing protected manifest receipt is invalid")
    return manifest
...
def assert_no_generated_outputs(paths: ProofRecoveryPaths) -> None:
    if any(os.path.lexists(path) for path in (*output_paths(paths).values(), paths.output)):
        raise proof.AbortGuardError(
            "generated output exists without an authorizing attestation"
        )


def remove_authorized_outputs(
    paths: ProofRecoveryPaths, attestation: proof.BaselineAttestation
) -> None:
    for name, path in output_paths(paths).items():
        if not os.path.lexists(path):
            continue
        content = _read_proof_bytes(path, _MAX_DATA_OUTPUT_BYTES, 0o600)
        if sha256_bytes(content) != attestation.output_sha256[name]:
            raise proof.AbortGuardError(
                "generated output bytes are not authorized for rollback"
            )
        try:
...
def verify_attestation(
    ...
    *,
    ...
    if (
        ...
        raise proof.AbortGuardError(
            "attestation paths do not bind the current recovery paths"
        )
    proof_mode = receipt.mode if receipt.original_sha256 == receipt.proof_sha256 else 0o600
    if (
        sha256_bytes(_read_proof_bytes(paths.env_file, 256 * 1024, proof_mode))
        != receipt.proof_sha256
        or sha256_bytes(_read_proof_bytes(paths.backup_path, 256 * 1024, 0o600))
        != receipt.original_sha256
    ):
        raise proof.AbortGuardError("proof env or backup drifted from its receipt")
    manifest = protected_bytes(paths)
    if attestation.output_sha256["protected-artifacts-before.txt"] != sha256_bytes(
        manifest
    ):
        raise proof.AbortGuardError("protected manifest drifted from attestation")
    for name, path in output_paths(paths).items():
        if sha256_bytes(_read_proof_bytes(path, _MAX_DATA_OUTPUT_BYTES, 0o600)) != (
            attestation.output_sha256[name]
        ):
            raise proof.AbortGuardError("attested output drifted")
    expected_result = result_bytes_from_attestation(attestation)
    require_generated_bounds({}, expected_result)
    if require_result or os.path.lexists(paths.output):
        try:
            exact = read_exact_regular_bytes(paths.output, expected_result)
        except CaptureSchemaError as error:
            raise proof.AbortGuardError("result bytes do not match attestation") from error
        if not exact:
            raise proof.AbortGuardError("result bytes do not match attestation")


def verify_result_bytes(attestation: proof.BaselineAttestation, value: bytes) -> None:
