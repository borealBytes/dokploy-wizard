...
def _snapshot(env_file: Path, state_dir: Path) -> dict[str, JsonValue]:
    ...
    return {
        ...
        "images": images,
        "schema_version": 1,
        "tailscale": {"identifiers": list(namespace.tailscale)},
...
def _api(hostname: str, token: str | None, path: str, body: dict[str, str] | None = None) -> dict[str, JsonValue] | list[JsonValue]:
    ...
    request_value = request.Request(f"https://127.0.0.1{path}", data=data, headers=headers, method="POST" if body else "GET")
...
def _templates(hostname: str, token: str) -> list[dict[str, JsonValue]]:
    raw = _api(hostname, token, "/api/v2/templates?limit=100&offset=0")
    if not isinstance(raw, list):
        raise ValueError("Coder template API must return a list")
...
def _workspaces(hostname: str, token: str, container: str, templates: list[dict[str, JsonValue]]) -> list[dict[str, JsonValue]]:
    raw = _api(hostname, token, "/api/v2/workspaces?q=&limit=100&offset=0")
    if not isinstance(raw, dict):
        raise ValueError("Coder workspace API returned an invalid response")
    workspace_values = raw.get("workspaces")
...
def _primary_pointer(container: str, token: str, workspace: str, template: str, version: str) -> dict[str, JsonValue]:
    ...
    result = subprocess.run(["docker", "exec", "-e", f"CODER_SESSION_TOKEN={token}", container, "/opt/coder", "ssh", workspace, "--", "python3", "-c", script], check=False, capture_output=True, text=True)
...
def _builds(hostname: str, token: str, workspaces: list[dict[str, JsonValue]]) -> list[dict[str, JsonValue]]:
    ...
    for workspace in workspaces:
        ...
        raw = _api(hostname, token, f"/api/v2/workspaces/{workspace_id}/builds?limit=100&offset=0")
        if not isinstance(raw, list):
            raise ValueError("Coder build API must return a list")
        ...
        inventories.append({
            ...
            "total": len(items),
            "pages": [{"offset": 0, "items": items}],
        })
    return inventories


def _secrets(hostname: str, token: str, user_id: str) -> list[dict[str, JsonValue]]:
    raw = _api(hostname, token, f"/api/v2/users/{user_id}/secrets")
    if not isinstance(raw, list):
        raise ValueError("Coder secrets API must return a list")
    return [{"id": _field(item, "id"), "name": _field(item, "name"), "environment_variable": _field(item, "env_name"), "description": _field(item, "description")} for item in raw]
...
def _page(items: list[dict[str, JsonValue]]) -> dict[str, JsonValue]:
    return {"total": len(items), "pages": [{"offset": 0, "items": items}]}
