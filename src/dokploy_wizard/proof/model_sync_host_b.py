...
def _workspace_json(container: str, token: str, workspace: str, script: str, payload: str, args: tuple[str, ...], label: str) -> JsonValue:
    command = ["docker", "exec", "-i", "-e", "CODER_URL=http://127.0.0.1:3000", container, "sh", "-c", "IFS= read -r CODER_SESSION_TOKEN; export CODER_SESSION_TOKEN; exec /opt/coder \"$@\"", "sh", "ssh", "--disable-autostart", workspace, "--", "node", "-e", script, *args]
    try:
