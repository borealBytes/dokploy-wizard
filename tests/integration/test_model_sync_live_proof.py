...
def test_upgrade_resume_receipt_orchestrates_blocked_then_strict_result(tmp_path: Path) -> None:
    ...
    binding = model_sync_upgrade_host_a.UpgradeHostABinding(
        ...
        proof_env_file=tmp_path / "task1-upload.env",
        proof_context_file=tmp_path / "task1-proof-context.json",
    )
    events: list[str] = []
    template_names = (
...
def test_model_inventory_normalizes_like_legacy_template(
    ...
    assert captured["command"][3:5] == ["-e", "CODER_URL=http://127.0.0.1:3000"]
